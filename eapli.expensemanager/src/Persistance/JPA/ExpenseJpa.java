...
public class ExpenseJpa extends JpaGeneric<Expense, Long> implements IExpenseRepository {
    ...
    public List<Expense> getAllExpensesByType(int month, int year, ExpenseType type) {
        EntityManager em = getEntityManager();
        //Query q = em.createQuery("SELECT exp FROM Expense exp WHERE exp.dateOccurred >= :start AND exp.dateOccurred <= :end ORDER BY :type");
	Query q = em.createQuery("SELECT exp FROM Expense exp WHERE exp.dateOccurred >= :start AND exp.dateOccurred <= :end AND exp.expType.id = :id ORDER BY exp.expType.id");
//        Calendar start = DateTime.newCalendarDate(year, month, 1);
//        Calendar end = DateTime.newCalendarDate(year, month, 31);
